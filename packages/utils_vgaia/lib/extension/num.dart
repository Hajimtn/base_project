...
import 'package:utils_vgaia/extension/extension.dart';
...
extension CurrencyFormat on num {
  ...
  num truncateToDecimalPlacesRound(int fractionalDigits) {
    String? value = this.toString().split('.').tryGet(1);
  ...
  DateTime convertToDate({DateTime? defaultDate}) {
    try {
      return DateFormat('yyyyMMdd').parse(this.toString());
    } catch (e) {
  ...
  int countDecimalPlaces() {
    String? decimalPart = this.toString().split('.').tryGet(1);
